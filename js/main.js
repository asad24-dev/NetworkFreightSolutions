...
jQuery(document).ready(function($) {
	...
	var siteSelect = function() {
		if ($('body#tracking-page').length > 0) {
			return;
		}
		...
		$('select').each(function(){
			...
			$wrap.find('label').hide();
			$this.hide();
			$this.wrap('<div class="select-wrap"></div>');
			$this.after('<span class="arrow-down"></span><div class="selected"><a href="#">'+$name+'</a></div>');
			
			var $ul = $('<ul class="drop-down"></ul>');
			$this.find('option').each(function(){
				var $this_option = $(this);
				$ul.append('<li><a href="#" data-value="'+$this_option.val()+'">'+$this_option.html()+'</a></li>');
			});
			$this.closest('.select-wrap').append($ul);
		});

		$(document).on('click', '.arrow-down', function(e){
			var $this = $(this);
			if ( $this.closest('.select-wrap').hasClass('active') ) {
		...
		$(document).on('click', '.select-wrap .drop-down a', function(e){
			var $this = $(this),
				$value = $this.data('value'),
				$parent = $this.closest('.select-wrap');

			$parent.find('select').val($value);
			$parent.find('.selected > a').html($this.html());
